...
void USART2_IRQHandler(void)                	//串口2中断服务程序
{
	...
	if(USART_GetITStatus(USART2, USART_IT_RXNE) != RESET)  //接收中断(接收到的数据必须是0x0d 0x0a结尾)
	{
		...
		if((Res==0xFE)&&(USART_RX_STA==0)){USART_RX_BUF[USART_RX_STA++]=Res;}
		if((Res==0x04)&&(USART_RX_STA==1)){USART_RX_BUF[USART_RX_STA++]=Res;}
		if(USART_RX_STA>=2&&(USART_RX_STA<=9))
			USART_RX_BUF[USART_RX_STA++]=Res;
//		XS_x=((((int16_t)USART_RX_BUF[3]) << 8) | USART_RX_BUF[2]);
//		XS_y=((((int16_t)USART_RX_BUF[5]) << 8) | USART_RX_BUF[4]);
